...
currentAccount: any;
    ...
    loadAll() {
        this.benefitService.query({
            page: this.page - 1,
            size: this.itemsPerPage,
            sort: this.sort()}).subscribe(
                (res: HttpResponse<Benefit[]>) => this.onSuccess(res.body, res.headers),
    ...
    transition() {
        this.router.navigate(['/benefit'], {queryParams:
            {
                page: this.page,
                size: this.itemsPerPage,
    ...
    trackId(index: number, item: Benefit) {
        return item.id;
    }
