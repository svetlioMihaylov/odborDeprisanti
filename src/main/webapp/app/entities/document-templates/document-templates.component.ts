...
@Component({
    selector: 'jhi-document-templates',
...
currentAccount: any;
    ...
    loadAll() {
        this.documentTemplatesService.query({
            page: this.page - 1,
            size: this.itemsPerPage,
            sort: this.sort()}).subscribe(
                (res: HttpResponse<DocumentTemplates[]>) => this.onSuccess(res.body, res.headers),
    ...
    transition() {
        this.router.navigate(['/document-templates'], {queryParams:
            {
                page: this.page,
                size: this.itemsPerPage,
    ...
    trackId(index: number, item: DocumentTemplates) {
        return item.id;
    }
    ...
    registerChangeInDocumentTemplates() {
        this.eventSubscriber = this.eventManager.subscribe('documentTemplatesListModification', (response) => this.loadAll());
    }
