...
currentAccount: any;
    ...
    loadAll() {
        this.financialDetailsService.query({
            page: this.page - 1,
            size: this.itemsPerPage,
            sort: this.sort()}).subscribe(
                (res: HttpResponse<FinancialDetails[]>) => this.onSuccess(res.body, res.headers),
    ...
    transition() {
        this.router.navigate(['/financial-details'], {queryParams:
            {
                page: this.page,
                size: this.itemsPerPage,
    ...
    trackId(index: number, item: FinancialDetails) {
        return item.id;
    }
    registerChangeInFinancialDetails() {
        this.eventSubscriber = this.eventManager.subscribe('financialDetailsListModification', (response) => this.loadAll());
    }
