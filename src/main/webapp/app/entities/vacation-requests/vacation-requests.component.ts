...
@Component({
    selector: 'jhi-vacation-requests',
...
currentAccount: any;
    ...
    loadAll() {
        this.vacationRequestsService.query({
            page: this.page - 1,
            size: this.itemsPerPage,
            sort: this.sort()}).subscribe(
                (res: HttpResponse<VacationRequests[]>) => this.onSuccess(res.body, res.headers),
    ...
    transition() {
        this.router.navigate(['/vacation-requests'], {queryParams:
            {
                page: this.page,
                size: this.itemsPerPage,
    ...
    trackId(index: number, item: VacationRequests) {
        return item.id;
    }
    registerChangeInVacationRequests() {
        this.eventSubscriber = this.eventManager.subscribe('vacationRequestsListModification', (response) => this.loadAll());
    }
