...
currentAccount: any;
    ...
    loadAll() {
        this.workDetailsService.query({
            page: this.page - 1,
            size: this.itemsPerPage,
            sort: this.sort()}).subscribe(
                (res: HttpResponse<WorkDetails[]>) => this.onSuccess(res.body, res.headers),
    ...
    transition() {
        this.router.navigate(['/work-details'], {queryParams:
            {
                page: this.page,
                size: this.itemsPerPage,
    ...
    trackId(index: number, item: WorkDetails) {
        return item.id;
    }
    registerChangeInWorkDetails() {
        this.eventSubscriber = this.eventManager.subscribe('workDetailsListModification', (response) => this.loadAll());
    }
